...
WARHAMMER_CHARACTERS = {
    ...
    "novaris": {
        ...
        "description": (
            ...
            "Вслушайся в его видения, и перед тобой откроется множество возможностей."
        ),
...
def button_callback(update, context):
    ...
    if data.startswith("select_"):
        ...
        bot.send_animation(
            ...
            caption=f"*{escape_md_v2(char['display_name'])}*\n{escape_md_v2(char['description'])}",
            parse_mode=ParseMode.MARKDOWN_V2,
            reply_markup=markup
        )
        ...
        temp_msg = bot.send_animation(
            chat_id=chat_id,
            animation=char["gif_url"],
            caption="_Голос Императора зовёт в бой!_",
            parse_mode=ParseMode.MARKDOWN_V2
        )
        for generated in stream_deepseek_api(prompt, []):
            if generated.strip():
...
def dismiss_command(update, context):
    ...
    for char_id in active_characters[chat_id]:
        ...
        temp_msg = update.message.reply_text("_Император подводит итог..._", parse_mode=ParseMode.MARKDOWN_V2)
            ...
            if generated.strip():
                try:
                    bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=temp_msg.message_id,
                        text=f"> *Прощание с {escape_md_v2(char['display_name'])}:*\n{generated}",
                        parse_mode=ParseMode.MARKDOWN_V2
                    )
                except Exception as e:
...
def stats_command(update, context):
    ...
    update.message.reply_text(f"В недавней битве за Империум {len(msgs)} сообщений.", parse_mode=ParseMode.MARKDOWN_V2)
...
def summarize_command(update, context):
    ...
    temp_msg = update.message.reply_text("_Император суммирует бой..._", parse_mode=ParseMode.MARKDOWN_V2)
...
def text_message_handler(update, context):
    ...
    if awaiting_question.get(chat_id, False):
        ...
        msgs = get_last_messages_db(chat_id, thread_id, limit=10)
        temp_message = update.message.reply_text("_Слава Императору! Формирую ответ..._", parse_mode=ParseMode.MARKDOWN_V2)
    ...
    if chat_id in active_characters and active_characters[chat_id]:
        ...
        for char_id in active_characters[chat_id]:
            ...
            temp = bot.send_message(chat_id=chat_id, text="_Император направляет голос битвы..._", parse_mode=ParseMode.MARKDOWN_V2)
            full_response = ""
            for generated in stream_deepseek_api(full_prompt, msgs):
                ...
                try:
                    bot.edit_message_text(
                        ...
                        text=f"*{escape_md_v2(char['display_name'])}* отвечает:\n{generated}",
                        parse_mode=ParseMode.MARKDOWN_V2
                    )
                except Exception as e:
        ...
        if len(active_characters[chat_id]) > 1:
            for char_id in active_characters[chat_id]:
                ...
                temp = bot.send_message(chat_id=chat_id, text="_Император добавляет слово..._", parse_mode=ParseMode.MARKDOWN_V2)
                    ...
                    if generated.strip():
                        try:
                            bot.edit_message_text(
                                chat_id=chat_id,
                                message_id=temp.message_id,
                                text=f"> *{escape_md_v2(char['display_name'])}* (комментарий):\n{generated}",
                                parse_mode=ParseMode.MARKDOWN_V2
                            )
                        except Exception as e:
